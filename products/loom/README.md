...
`loom-backend-llama` pins `llama-native-engine`, `llama-native-host`, and `llama-native-types` to published commit `b71dfaa16c77b7069259bd15add740b80f895017` of `delysis/llama-native-kit`. Builds therefore require fetching that exact Git revision unless dependencies are already cached. The pin includes the product-neutral raw branch-family API, reviewed NativeHost single-flight/cache policy, and joined native shutdown authority used by the desktop lifecycle. Changing it requires full consumer compatibility and real-GGUF revalidation.
