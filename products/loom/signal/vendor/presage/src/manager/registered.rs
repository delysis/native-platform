...
impl<S: Store> Manager<S, Registered> {
    ...
    /// Fetch one ACI's public identity for an explicit safety-number review.
    /// This does not save a trusted key, create a session, consume prekeys, or
    /// send a message. The caller owns comparison and any subsequent consent.
    pub async fn retrieve_identity_key(
        &mut self,
        aci: Aci,
    ) -> Result<libsignal_service::protocol::IdentityKey, Error<S::Error>> {
        let profile = self
            .identified_websocket(false)
            .await?
            .retrieve_profile_by_id(aci, None)
            .await?;
        let bytes = profile.identity_key.ok_or(Error::MissingKeyError(
            "Signal profile identity key".into(),
        ))?;
        Ok(libsignal_service::protocol::IdentityKey::decode(&bytes)?)
    }
