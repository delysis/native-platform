...
async function render(config: Partial<WorkspacePaneConfig> = {}, value = source.text) {
  ...
  const beforeRun = vi.fn(async () => source), onOpenDocument = vi.fn(), onChange = vi.fn(), onModelSelect = vi.fn(), onCompositionChange = vi.fn();
  mounted = mount(WorkspacePane, { target, props: {
    ...
    projectId: 'project', sessionId: 'session', source, value, documents: [source.summary, { ...source.summary, document_id: 'result', relative_path: 'Runs/1/Answer.md', title: 'Answer' }], beforeRun, onOpenDocument, onChange, modelLabel: 'Gemma', onModelSelect, onCompositionChange
  } });
