...
- `fte-speech-types`: protocol-neutral STT/TTS contracts, privacy and routing
  policy, model and voice descriptors, event tickets, cancellation, usage,
  typed errors, and the backend trait.
- `fte-speech-platform`: deterministic platform adapter discovery and bounded
  aggregation of evidence from native or embedded runtime probes.
- `fte-speech-router`: hard capability/privacy gates followed by deterministic
  policy ordering, exact model/voice routing, and complete rejection receipts.
- `fte-speech-gateway`: a Tauri-independent registry and execution service that
...
- `fte-speech-parakeet`: an executable, embedded Parakeet Realtime EOU 120M
...
The current macOS launched-app artifact is produced by setting
`FTE_APPLE_TTS_SMOKE_RECEIPT` to a receipt path. The app synthesizes one fixed
sentence, writes a sibling WAV plus a versioned JSON receipt, and exits. This
hook is inert during every normal launch and is not a user-facing control. The
smoke dispatches through the plugin-managed `SpeechGateway`, rather than
calling the Apple adapter directly, so it verifies the actual embedding path.

The embedded STT artifact is produced by setting
`FTE_PARAKEET_STT_SMOKE_RECEIPT` to a receipt path and
`FTE_SPEECH_TEST_WAV` to a real English WAV. The app waits for asynchronous
backend registration, routes the audio through plugin-managed state, records
the exact model/route/network/usage/event evidence, and exits. The verified
receipt at `/tmp/fte-parakeet-tauri-smoke-resident.json` reported
`real_local_inference = true`, `network = never`, `model_load_ms = 0`, and
exactly one terminal event. The real gateway test also streams PCM through the
audio sink and cancels one request without affecting its completing peer.
