...
use fte_speech_router::{SpeechRouteError, SpeechRoutePlan, SpeechRouter};
...
impl From<SpeechRouteError> for SpeechGatewayError {
    fn from(error: SpeechRouteError) -> Self {
        Self::Route { error }
    }
}

impl From<SpeechError> for SpeechGatewayError {
...
pub struct SpeechGatewayStatus {
...
impl std::fmt::Debug for SpeechGateway {
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        formatter
            .debug_struct("SpeechGateway")
            .field("target", &self.target)
            .finish_non_exhaustive()
    }
}

impl Default for SpeechGateway {
    fn default() -> Self {
...
impl SpeechGateway {
    ...
    pub fn register_backend(
        ...
        descriptor
            .validate()
            .map_err(|error| SpeechGatewayError::BackendInvalid {
                detail: error.to_string(),
            })?;
        let mut backends = self
            .backends
            .write()
            .map_err(|_| SpeechGatewayError::StateUnavailable)?;
        if backends.contains_key(&descriptor.id) {
            return Err(SpeechGatewayError::BackendDuplicate {
    ...
    pub fn status(&self) -> Result<SpeechGatewayStatus, SpeechGatewayError> {
        Ok(SpeechGatewayStatus {
            target: self.target.clone(),
            backends: self.descriptors()?,
        })
    }

    pub fn descriptors(&self) -> Result<Vec<SpeechBackendDescriptor>, SpeechGatewayError> {
        let backends = self
            .backends
            .read()
            .map_err(|_| SpeechGatewayError::StateUnavailable)?;
    ...
    pub fn snapshot(&self) -> Result<PlatformCapabilitySnapshot, SpeechGatewayError> {
    ...
    ) -> Result<TranscriptionTicket, SpeechGatewayError> {
    ...
    ) -> Result<SynthesisTicket, SpeechGatewayError> {
    ...
    pub async fn shutdown(&self) -> Result<(), SpeechGatewayError> {
        let backends = self
            .backends
            .read()
            .map_err(|_| SpeechGatewayError::StateUnavailable)?
        ...
        if !failures.is_empty() {
            return Err(SpeechGatewayError::Shutdown { failures });
        }
        Ok(())
    }

    fn backend(&self, backend_id: &str) -> Result<Arc<dyn SpeechBackend>, SpeechGatewayError> {
        self.backends
            .read()
            .map_err(|_| SpeechGatewayError::StateUnavailable)?
            .get(backend_id)
            .cloned()
            .ok_or_else(|| SpeechGatewayError::BackendMissing {
...
#[must_use]
pub fn service_error(request_id: &SpeechRequestId, error: SpeechGatewayError) -> SpeechError {
    SpeechError {
...
mod tests {
    ...
    async fn service_plans_pins_and_executes_registered_backend() {
        let gateway = SpeechGateway::default();
    ...
    fn duplicate_backend_registration_fails_closed() {
        ...
        assert!(matches!(
            ...
            Err(SpeechGatewayError::BackendDuplicate { .. })
        ));
    }
    ...
    async fn shutdown_attempts_every_backend_and_reports_all_failures() {
        ...
        let SpeechGatewayError::Shutdown { failures } = error else {
