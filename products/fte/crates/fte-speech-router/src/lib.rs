...
fn evaluate_transcription(
    ...
    if !matches!(
        request.diarization,
        fte_speech_types::DiarizationPolicy::Disabled
...
fn evaluate_synthesis<'a>(
    ...
    language_supported(request.language.as_deref(), capability, backend)?;
    if matches!(request.input, fte_speech_types::SynthesisInput::Ssml { .. }) && !features.ssml {
        return Err(rejection(
            backend,
            Some(capability),
...
mod tests {
    ...
    fn transcription() -> TranscriptionRequest {
        TranscriptionRequest {
            ...
            diarization: fte_speech_types::DiarizationPolicy::Disabled,
