//! Tauri 2 embedding surface for the independent Free Token Energy speech service.
...
impl Builder {
    ...
    pub fn build<R: Runtime>(self) -> TauriPlugin<R> {
        ...
        PluginBuilder::new("free-token-energy-speech")
...
pub trait FreeTokenEnergySpeechExt<R: Runtime> {
    fn free_token_energy_speech(&self) -> Arc<SpeechGateway>;
}

impl<R: Runtime, T: Manager<R>> FreeTokenEnergySpeechExt<R> for T {
    fn free_token_energy_speech(&self) -> Arc<SpeechGateway> {
...
) -> Result<SpeechGatewayStatus, SpeechGatewayError> {
    state.speech.status()
}
