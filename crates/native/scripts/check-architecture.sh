...
contract_manifests="$(grep -l 'w1-platform-contracts' crates/native/crates/*/Cargo.toml || true)"
if [ "$contract_manifests" != "$w1_manifest" ]
then
  echo "W1 contract dependency must have exactly one manifest owner: $w1_manifest" >&2
  exit 1
fi
if ! grep -Fq "git = \"$w1_contract_url\", rev = \"$w1_contract_rev\", optional = true" "$w1_manifest"
then
  echo "W1 contract dependency must use the approved exact optional revision" >&2
  exit 1
fi
if grep -En 'w1-platform-contracts.*(branch|tag)[[:space:]]*=' crates/native/crates/*/Cargo.toml
then
  echo "moving W1 contract branch or tag dependencies are forbidden" >&2
  exit 1
fi
if ! grep -Fq "platform-vertical-fixtures-v0 = { git = \"$w1_contract_url\", rev = \"$w1_vertical_rev\", optional = true }" "$w1_manifest"
then
  echo "W1 vertical dependency must use the approved exact optional revision" >&2
  exit 1
fi
if ! grep -Fq "platform-contracts-v0-vertical = { package = \"platform-contracts-v0\", git = \"$w1_contract_url\", rev = \"$w1_vertical_rev\", optional = true }" "$w1_manifest"
then
  echo "W1 vertical contract types must use the matching approved exact optional revision" >&2
  exit 1
fi
if [ "$(grep -o 'w1-platform-contracts' crates/native/crates/*/Cargo.toml | wc -l | tr -d ' ')" -ne 3 ]
then
  echo "unexpected W1 dependency declaration count" >&2
  exit 1
fi
if ! grep -Fq "git+$w1_contract_url?rev=$w1_contract_rev#$w1_contract_rev" Cargo.lock
then
  echo "Cargo.lock must resolve the exact approved W1 contract revision" >&2
  exit 1
fi
if ! grep -Fq "git+$w1_contract_url?rev=$w1_vertical_rev#$w1_vertical_rev" Cargo.lock
then
