...
if grep -Ev '^[a-z0-9-]+$' "$package_file"; then
  echo "portable package list contains an invalid entry" >&2
  exit 1
fi

if [[ "$(sort "$package_file" | uniq -d | wc -l | tr -d ' ')" != 0 ]]; then
...
if grep -Eiq '(^|-)tauri($|-)|gui|desktop' "$package_file"; then
