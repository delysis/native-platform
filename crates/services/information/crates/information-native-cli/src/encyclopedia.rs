...
use crate::{
    CliResult, EncyclopediaArticleArgs, EncyclopediaSearchArgs, EncyclopediaSourceArgs, fail,
    json_value,
};
use information_native_backend_encyclopedia::{
    ENCYCLOPEDIA_ARTICLE_COLLECTION, EncyclopediaBackend, EncyclopediaBackendConfig,
};
...
pub(crate) fn search(args: &EncyclopediaSearchArgs) -> CliResult<serde_json::Value> {
    validate_values("subjects", &args.subjects, 128, 1_024)?;
    validate_values("document ids", &args.document_ids, 128, 32)?;
    let fields = compiled_fields(&args.fields, 8)?;
...
pub(crate) fn article(args: &EncyclopediaArticleArgs) -> CliResult<serde_json::Value> {
    let mounted = mount(&args.source)?;
    let request = LookupRequest {
        ...
        purpose: args.purpose.into(),
        collection: Some(ENCYCLOPEDIA_ARTICLE_COLLECTION.to_string()),
        key: args.article_id.clone(),
        max_context_chars: args.max_context_chars,
        timeout_ms: args.timeout_ms,
    };
...
fn mount(args: &EncyclopediaSourceArgs) -> CliResult<MountedEncyclopedia> {
    ...
    let backend: Arc<dyn ResourceBackend> = Arc::new(EncyclopediaBackend::open(config)?);
    let router = RetrievalRouter::from_backends([backend])?;
    Ok(MountedEncyclopedia {
        router,
        resource_id,
        release_id,
        representation_id,
